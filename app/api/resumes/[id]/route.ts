...
export async function PUT(request: Request, { params }: { params: Promise<{ id:string }> }) {
  try {
    ...
    await prisma.$transaction(async tx=>{
      await tx.resume.update({where:{id},data:{title:d.title,template:d.template,fontFamily:d.fontFamily,colorTheme:d.colorTheme,summary:d.summary,profile:{upsert:{create:d.profile,update:d.profile}}}});
